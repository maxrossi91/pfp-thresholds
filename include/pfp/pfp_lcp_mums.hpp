...
public:
    ...
    pfp_lcp(pf_parsing &pfp_, std::string filename, bool rle_ = false) : 
                pf(pfp_),
                min_s(1, pf.n),
                pos_s(1,0),
                head(0),
                rle(rle_)
    {
        ...
        std::string outfile = filename + std::string(".mums.lcp");
...
private:
    ...
    inline void update_lcp(int_t val, size_t pos)
    {
        if (length == 2)
            s_lcp = val;
